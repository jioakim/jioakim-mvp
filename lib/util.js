...
//----------------------------------------------------------------
//----------GET A RECORD WITH SPECIFIC FIRST NAME-----------------
exports.handleGetFirstName = function(req, res) {
  FirstName.findOne({firstName: req.params.firstname}, function(err, firstName){
    if (err) {
      console.log(err);
    }
    if (firstName) {
...
//----------------------------------------------------------------
//----------POST NEW FIRST NAME TO DATABASE-----------------------
exports.handleNewUserPost = function(req, res) {
  var newFirstName = req.params.newuser;
  FirstName.findOne({firstName: newFirstName}, function(err, firstName){
    if (err) {
      console.log(err);
    }
    if (firstName) {
      res.send({newUser:'exists'});
    } else {
      var veryBigNumber = 999999999;
      var rand = Math.floor(Math.random() * veryBigNumber);
      var fn = newFirstName;
      var insert = [{rand:rand, firstName:fn}];
      FirstName.collection.insertMany(insert, function(err, r){
...
exports.handleUserUpdate = function(req, res) {
  var updateFirstNameId = req.params.firstname_id;
  FirstName.findById(updateFirstNameId, function(err, firstName){
    if (err) {
      res.send(err);
    }
